...
/* PLANETARY EPHEMERIDES - DE430 MODEL (JPL) (POSITION ONLY)
 --------------------------------------------------------------------------------------------------------------------
 Returns position vectors for any of 11 bodies in the solar system in the International Celestial Reference Frame 
 (ICRF) relative to Solar System Barycenter. Moon position vector given for Geocentric Celestial Reference Frame 
 (GCRF). Ephemerides based on JPL DE430 model.
 ...
 Line to call .mex file from Matlab: 
      ...
      [ephemerides] = planet_ephemerides_c(MJD_TT,transpose(DE430coef),bodies);
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    double tbar, MJDint1, MJDint2, xcoef[14], ycoef[14], zcoef[14], Tn[14], TNM1, TNP1, TN; 
    double R_ICRFx = 0, R_ICRFy = 0, R_ICRFz = 0;
    ...
    for (b = 0; b < dims_bodies[1]; b++) /*Loop for planetary body*/
    {
        ...
        R_ICRFy = 0;
        R_ICRFz = 0;
        for (i = 0; i < numcoef[p]; i++) /*Loop to compute position vector components*/
        {
